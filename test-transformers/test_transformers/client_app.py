...
class FlowerClient(NumPyClient):
    def __init__(self, net, partition_id, trainloader, testloader, local_epochs, cpu_time):
        self.net = net
    ...
    def fit(self, parameters, config):
        print(f"client {self.partition_id}: start training with {self.cpu_time} of cpu time")
        set_weights(self.net, parameters)
        train(self.net, self.trainloader, epochs=self.local_epochs, device=self.device)
        return get_weights(self.net), len(self.trainloader), {"cpu_time": self.cpu_time}
...
def client_fn(context: Context, config: dict):
    ...
    cpu_time = clients[randint(0,len(clients)-1)]["cpu_time"]
    # Return Client instance
    return FlowerClient(net, partition_id,trainloader, valloader, local_epochs, cpu_time).to_client()
